...
This setup is very similar to a targeted \ilc{FGSM}. So natually, we want to get to the boundary of the $x$ and $x'$. However with no access to \ilc{loss} function, what we can do is to do a \ilc{binary search} between $x$ and $x'$ and make it $x_{\text{new}}$, and then estimate the gradient of $x_{\text{new}}$, move along such estimated gradient for a short step, then do another \ilc{binary search} until $S(x') > 0$.

For the gradient estimation, the author proposed a function of:


\begin{equation}
    \widetilde{\nabla S}(x', \delta) = \frac{1}{B} \limit \sum^{B}_{b=1} \phi (x' + \delta u_b) u_b
    \label{hsj_g_est}
\end{equation}

Where $\phi(x') = \text{sign}(S(x'))$, which we have get by simply asking the model to predict $x'$. And $u_b$ are group of random vectors around $x'$. We than ignore the random vertors that is close to being parallel to the decision boundary (in the case that a decision boundary is not a plane, close to the tangent of the boundary) -- we may identify them by looking at vectors of similar direction but have different $\phi(x')$ output when added to $x'$. Then we look at the leftover random vectors to converge an estimation of the boundary like demoed in [\figurename{\ \ref{hsj_grad}}].


\begin{figure}[H]
    \centering
    \includegraphics[width = 0.6\textwidth]{{fig/hsj_grad.png}}
    \caption{Using random vectors to estimate boundary \protect{\cite{cite:hsj_talk}}}
    \label{hsj_grad}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width = 0.8\textwidth]{{fig/hsj_exp.png}}
    \caption{\ilc{HSJ} running with recommended and fast params setting on \textit{MINIST}}
    \label{hsj_exp}
\end{figure}
...
\ilc{Feature Collision}\cite{cite:fc_paper} is an algorithm I read and understood but did not actually implement, mostly due to time constraint. \ilc{Feature Collision} is a \textit{posioning} attack algorithm, which is usually considered to be impartical as it is highly unlikely for an attack to have access to the training phase of a model.


But \ilc{Feature Collision} is implemented with concept of \ilc{clean label}, as not the attack but a third-party will label the ``poisoned'' image. This increases the likelihood of the poisoned image entering the training set of the model as it is commom practice to scrape internet for training data.

So, in a general term, \ilc{Feature Collision} \textit{collides} the feature between a base class to a target class, making adversarial example that carrying the semantic the base class but full of the features of the target class. Since it is indistinguishable to human eyes, if it enters to training set of a model, the result can be catastrophic like demoed in [\figurename{\ \ref{fc_eff}}].


\begin{figure}[H]
    \centering
    \includegraphics[width = 0.8\textwidth]{{fig/fc_eff.png}}
    \caption{The result of \ilc{Feature Collision} \protect{\cite{cite:fc_paper}}}
    \label{fc_eff}
\end{figure}

In the paper, the author also emphasis the effectiveness of \ilc{Feature Collision} as normally posioning attack will require control of labels or even having adversarial examples in to most or every batch of images, but \ilc{Feature Collision} complete an effective attack even with just one adversarial example -- a legitment \textit{one-shot kill} as named by the authors.
...
We believe this have something to do with the fact \ilc{SS} set the value of each pixel to be the mean value of the pixels around this pixel and it can make the pixel value become more continuous. As \ilc{HSJ} being on the decisions boundaries (implied by the low \textit{pertubation budget}),
Thus the mean value set by \ilc{SS} can used to vague the boundaries to defense.
...
\ilc{Neural Cleanse} is our implemented defense algorithm focus on Unlearning the backdoor by retraining，the algorithm is defend by train infected DNN to recognize correct labels. Here we specifically retrain the pattern of \ilc{Backdoor} insert as mentioned on Section $3.1.4$. Thus Against the \ilc{Backdoor} attack, \ilc{Neural Cleanse} can achieve a very good defense effect.\newline
By the graph, the Defensive Accuracy is $70.89\%$ (Note we train the whole \textit{MINIST} dataset), which is a very good defence accuracy. In this case, we do not want to compare with other attack algorithm, because the essence of Unlearning retraining is to allow the neural network to resist the influence of external patterns, so retraining is to use pattern to modify the data of the test set and then for training，therefore only \ilc{Backdoor} we implemented is pattern attack, that is why we only compare \ilc{Backdoor} to against \ilc{Neural Cleanse} on this section.
